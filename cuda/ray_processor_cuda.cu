    ...
    if (!valid) {
        ...
        if (false_alarm_rate > 0.f && rand_states != nullptr &&
            ...
            const float d_fa = rpmath::quantizeRange(
                min_range + curand_uniform(&rand_states[idx]) *
    ...
    const float * d_nir_in)
{
    ...
    rayProcessKernel<<<grid, kBlock, 0, static_cast<cudaStream_t>(stream)>>>(
        ...
        p.range_noise_min_std, p.range_noise_max_std,
        p.range_noise_reference_range, p.max_range,
        p.min_range, p.detection_range_10, p.detection_range_80,
        p.detection_range_10_d50, p.detection_range_80_d50,
        p.detection_rolloff, p.range_resolution,
