...
| `ray_mode` | `panels` | `panels` \| `raycast` | `panels` renders a perspective depth-panel rig on the GPU and resamples each beam from it. `raycast` casts every beam exactly (calibrated direction, true beam-origin parallax) against an ECM scene mirror — zero interpolation error, `laser_retro × cos(incidence)` drives reflectivity/signal (extended-Lambertian lidar equation, see [docs/MODEL_REFERENCES.md](docs/MODEL_REFERENCES.md)), no rendering involved (no anchor-sensor requirement). CPU/OpenMP; mirrors box/sphere/cylinder/plane/mesh visuals. |
